...
typedef struct{
	...
	unsigned long activation_time;	//Activation time, could get it from system but it will always be sorted
	enum STATES state;
} Task;
...
void fcfs(){
	...
	unsigned char i,first=0;//Assume first task in sorted array is in READY state
	sort(tasks,TASKS_LIMIT,1);
	for(i=0;i<TASKS_LIMIT;i++){
		printf("Task ID: %d ACTIVATION_TIME: %d STATE: %s\n",tasks[i].id,tasks[i].activation_time,(!tasks[i].state) ? "IDLE" : "READY");
	}
	for(i=0;i<TASKS_LIMIT;i++){
	...
	printf("FCFS scheduling result:\nID: %d ACTIVATION_TIME: %d STATE: RUN\n",tasks[first].id,tasks[first].activation_time);
}
...
void priority_scheduling(){
	...
	for(i=0;i<TASKS_LIMIT;i++){
		printf("Task ID: %d PRIORITY: %d STATE: %s\n",tasks[i].id,tasks[i].priority,(!tasks[i].state) ? "IDLE" : "READY");
	}
	...
	printf("---------------------------------------------------------------\n");
	tasks[first].state=RUN;
	printf("Piority scheduling result:\nID: %d PRIORITY: %d STATE: RUN\n",tasks[first].id,tasks[first].priority);
}
